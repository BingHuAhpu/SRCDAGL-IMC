function S = constructS_KNG(fea,options)
%	Usage:
%	S = constructS_KNG(fea,options)
%
...
%                                S_ij = exp(-norm(x_i - x_j)/2t^2). You are 
...
%      bSelfConnected  -   0 or 1. Indicates whether S(i,i) == 1. Default 0
...
if strcmpi(options.NeighborMode,'KNN') && (options.k > 0)
    ...
    if isfield(options,'bSemiSupervised') && options.bSemiSupervised
        ...
        S(options.semiSplit,options.semiSplit) = (Wsup>0)*options.SameCategoryWeight;
    end
...
if strcmpi(options.NeighborMode,'KNN') & (options.k == 0)
    switch lower(options.WeightMode)
        ...
        case {lower('HeatKernel')}
            S = EuDist2(fea,[],0);
            S = exp(-S/(2*options.t^2));
        case {lower('Cosine')}
            S = full(Normfea*Normfea');
        otherwise
            error('WeightMode does not exist!');
    end
